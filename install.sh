...
if test ! $(which node); then
  echo $sudoPW | sudo -S apt-get install -y ca-certificates curl gnupg
  echo $sudoPW | sudo -S mkdir -p /etc/apt/keyrings
  curl -fsSL https://deb.nodesource.com/gpgkey/nodesource-repo.gpg.key | echo $sudoPW | sudo -S gpg --dearmor -o /etc/apt/keyrings/nodesource.gpgA
  NODE_MAJOR=20
  echo "deb [signed-by=/etc/apt/keyrings/nodesource.gpg] https://deb.nodesource.com/node_14.x focal main" | echo $sudoPW | sudo -S tee /etc/apt/sources.list.d/nodesource.list
